...
@interface XTMarqueeView () <XTMarqueeViewTouchResponder>
...
@property (nonatomic, strong) XTMarqueeViewTouchReceiver *touchReceiver;
...
- (void)resetAll {
    ...
    if (_direction == XTMarqueeViewDirectionLeftward) {
...
- (void)scrollTimerDidFire:(NSTimer *)timer {
    ...
    dispatch_async(dispatch_get_main_queue(), ^() {
        @strongify(self)
        if (self.direction == XTMarqueeViewDirectionLeftward) {
            [self moveToNextDataIndex];
...
- (void)resetTouchReceiver {
    if (_touchEnabled) {
        if (!_touchReceiver) {
            self.touchReceiver = [[XTMarqueeViewTouchReceiver alloc] init];
...
@implementation XTMarqueeViewTouchReceiver
