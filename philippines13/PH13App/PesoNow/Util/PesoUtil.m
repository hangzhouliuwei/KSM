...
+ (void)showToast:(NSString *)str {
    ...
    [QMUITips showWithText:str inView:[self keyWindow] hideAfterDelay:2];
}
...
+ (void)showToast:(NSString *)str time:(NSTimeInterval)time {
    ...
    [QMUITips showWithText:str inView:[self keyWindow] hideAfterDelay:time];
}
...
+ (NSString *)stringWithNumber:(NSNumber *)number {
    NSString *value = [NSString stringWithFormat:@"%lf", number.doubleValue];
...
+ (NSDecimalNumber *)formatNumber:(NSNumber *)number {
    NSString *value = [NSString stringWithFormat:@"%.2lf", number.doubleValue];
...
+ (NSString *)setNotSeeText:(NSString *)text first:(NSInteger )first last:(NSInteger)last {
    ...
    if (text.length <= 6 && text.length>1) {
        ...
        newText = [NSString stringWithFormat:@"%@**%@", firstString,lastString];
...
+ (void)showMBProgress:(UIView *)targetView message:(NSString *)msg
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:targetView ? targetView : [self keyWindow]  animated:YES];
...
{
    dispatch_async(dispatch_get_main_queue(), ^{
       [MBProgressHUD hideHUDForView:targetView? targetView:[self keyWindow] animated:YES];
    });
}
...
+ (UIViewController *)findVisibleViewController {
    UIViewController* currentViewController = [self keyWindow].rootViewController;
