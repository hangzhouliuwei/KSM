...
@implementation PesoDeviceTool
+ (NSString *)version
{
    NSDictionary *infoDic = [[NSBundle mainBundle] infoDictionary];
    NSString *appVersion = [infoDic objectForKey:@"CFBundleShortVersionString"];
    return NotNil(appVersion);
}
...
+ (NSString *)buildVersion {
    ...
    return NotNil(appBuildVersion);
}
//
+ (NSString *)systemVersion {
    NSString *systemVersion = [[UIDevice currentDevice] systemVersion];
    return NotNil(systemVersion);
}
+ (NSString *)idfa {
...
+ (NSString *)idfv {
    NSString * currentDeviceUUIDStr = [SAMKeychain passwordForService:[NSBundle mainBundle].bundleIdentifier account:@"uuid"];
    if (currentDeviceUUIDStr == nil || [currentDeviceUUIDStr isEqualToString:@""])
    {
        NSUUID * currentDeviceUUID  = [UIDevice currentDevice].identifierForVendor;
        currentDeviceUUIDStr = currentDeviceUUID.UUIDString;
        [SAMKeychain setPassword: currentDeviceUUIDStr forService:[NSBundle mainBundle].bundleIdentifier account:@"uuid"];
    }
...
+ (NSString *)packageName {
    ...
    return NotNil(appName);
}

+ (NSString *)bundleId {
    NSString *bundleId = [[NSBundle mainBundle] bundleIdentifier];
    return NotNil(bundleId);
}
...
+ (NSString *)standardFormateMAC:(NSString *)MAC {
    ...
    return NotNil([formateMAC uppercaseString]);
}
...
{
    //读取账号中保存的内容
    NSString *fistTimeStr = [[NSUserDefaults standardUserDefaults] objectForKey:@"KEY_FistTime"];
    if (!NotNil(fistTimeStr)){
        //nil
        NSString *string = [self getTime];//获取当前时间
        [[NSUserDefaults standardUserDefaults] setObject:string forKey:@"KEY_FistTime"];
...
{
    NSDictionary *zotenaismNcDic = @{
                                    @"chysthirteenalidesNc":NotNil([self getAvailableDiskSize]),
                                    @"ebdithirteencNc":NotNil([self getTotalDiskSize]),
                                    @"sclithirteenmazelNc":NotNil([self getTotalMemorySize]),
                                    @"hiacthirteenkNc":NotNil([self getAvailableMemorySize]),
                                    };
    NSDictionary *battery_statusDic = @{
                                         @"delathirteensseNc":@([self batteryLevel]),
                                         @"battery_status":@([self getBatteryStatus] ? 1 : 0 ),
                                         @"akavthirteenitNc":@([self isCharging] ? 1 : 0)
                                       };
    NSDictionary *hardwareDic = @{
                                  @"xeotthirteenimeNc":NotNil([self systemVersion]),
                                  @"prtuthirteenbercularNc":@"iPhone",
                                  @"bauvthirteenrihiNc":NotNil([self deviceName]),
                                  @"pemethirteenanceNc":@([self deviceHeight]),
                                  @"sttuthirteensNc":@([self deviceWidth]),
                                  @"soenthirteenoidNc":NotNil([self deviceSize]),
                                  @"terathirteenNc":@([self getDeviceProTime])
                                 };
    NSDictionary *watentyNcDic = @{
                                   @"brezthirteeninessNc":@"0",
                                   @"deisthirteenableNc":@([self isSimulator] ? 1 : 0),
                                   @"sinmthirteenanNc":@([self isJailbroken] ? 1 : 0)
                                 };
    NSDictionary *retenncounterNcDic = @{
                                      @"ovrethirteenxertNc": NotNil([self timeZone]),
                                      @"pltithirteenniferousNc":@([self isUseProxy] ? 1 : 0),
                                      @"sumethirteenrgibleNc":@([self isUseVPN] ? 1 : 0),
                                      @"consthirteenellorNc":NotNil([self getSIMCardInfo]),
                                      @"manithirteencideNc":NotNil([self idfv]),
                                      @"tuedthirteenoNc":NotNil([self deviceLanuage]),
                                      @"leelthirteenlingNc":NotNil([self networkStatus]),
                                      @"bahlthirteenykNc":@(1),
                                      @"deodthirteenulateNc":NotNil([self ip]),
                                      @"patuthirteenrageNc":NotNil([self idfa]),
                                     };
    
    NSDictionary *patenricentricNcDic = @{
                                          @"mitithirteenmeNc":NotNil([self getBSSID]),
                                          @"frscthirteenatiNc":NotNil([self getWifiName]),
                                          @"koobthirteenehNc":NotNil([self getBSSID]),
                                          @"uporthirteennNc":NotNil([self getWifiName]),
                                         };
    NSDictionary *eatenternizeNcDic = @{@"parithirteencentricNc":@[patenricentricNcDic]};
    
    NSDictionary *dic = @{
                          @"zoaithirteensmNc": zotenaismNcDic,
                          @"battery_status":battery_statusDic,
                          @"hardware":hardwareDic,
                          @"watythirteenNc":watentyNcDic,
                          @"rencthirteenounterNc":retenncounterNcDic,
                          @"eatethirteenrnizeNc":eatenternizeNcDic
                         };
