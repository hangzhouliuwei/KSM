...
@interface PesoWebVC ()<WKUIDelegate, WKNavigationDelegate,WKScriptMessageHandler>
@property(nonatomic, strong) WKWebView *webView;
@property(nonatomic, strong) UIProgressView *progressView;
@end

@implementation PesoWebVC
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    if (@available(iOS 14.0, *)) {
        ...
        NSArray * listMthod = @[@"thirteen001", @"thirteen002", @"thirteen003", @"thirteen004", @"thirteen005",@"thirteen006",];
...
- (void)loadWebview
{
    NSString *paraUrlString = AFQueryStringFromParameters([PesoRequestArguments getURLParam]).stringByRemovingPercentEncoding;
    NSString *urlStr = @"";
    if ([self.url hasSuffix:@"?"]) {
        urlStr = [NSString stringWithFormat:@"%@%@", self.url, [PesoUtil urlZhEncode:paraUrlString]];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    ...
    if([message.name isEqualToString:@"thirteen001"]) {//getPoint 获取埋点信息
        NSDictionary *dic = @{
        ...
        NSString *str = [NSString stringWithFormat:@"thirteen002(%@)",[dic yy_modelToJSONString]];
    ...
    else if([message.name isEqualToString:@"thirteen003"]) {//callPhoneMethod(String phoneNumber) H5 页面里的拨打电话
    ...
    else if([message.name isEqualToString:@"thirteen004"]) {//jumpToHome 回到 App 首页
        [[PesoRootVCCenter sharedPesoRootVCCenter] switchIndex:0];
    }
    else if([message.name isEqualToString:@"thirteen005"]) {//toGrade 调用 App 应用评分
        if(@available(iOS 14.0, *)) {
    ...
    else if([message.name isEqualToString:@"thirteen006"]) {//openAppstore(String appPkg) 跳转 Appstore
...
-(WKWebView *)webView{
    if(!_webView){
        _webView = [[WKWebView alloc] initWithFrame:CGRectMake(0, kNavBarAndStatusBarHeight, kScreenWidth, kScreenHeight - kNavBarAndStatusBarHeight - KBottomHeight)];
...
-(UIProgressView *)progressView{
    if(!_progressView){
        _progressView = [[UIProgressView alloc] initWithFrame:CGRectMake(0, kNavBarAndStatusBarHeight, kScreenWidth, 0.5)];
        _progressView.tintColor = ColorFromHex(0x29CB2D);
