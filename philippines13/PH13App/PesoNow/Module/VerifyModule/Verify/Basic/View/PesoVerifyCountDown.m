...
@interface PesoVerifyCountDown()
@property(nonatomic,strong) dispatch_source_t timer;
@property(nonatomic,strong) NSDateFormatter *dateFormatter;

@end

@implementation PesoVerifyCountDown
...
-(void)cmsCountDownWithtimeStamp:(long long)timeStamp completeBlock:(void (^)(NSInteger day,NSInteger hour,NSInteger minute,NSInteger second))completeBlock{
    ...
    if (_timer==nil) {
        long long value = timeStamp/1000;
        NSNumber *time = [NSNumber numberWithLongLong:value];
        //转换成NSTimeInterval,用longLongValue，防止溢出
        NSTimeInterval nsTimeInterval = [time longLongValue];
        __block int timeout = nsTimeInterval; //倒计时时间
        if (timeout!=0) {
            dispatch_queue_t queue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
            _timer = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0,queue);
            dispatch_source_set_timer(_timer,dispatch_walltime(NULL, 0),1.0*NSEC_PER_SEC, 0); //每秒执行
...
-(void)cmsCountDownWithSecond:(NSInteger)timeStamp completeBlock:(void (^)(NSInteger day,NSInteger hour,NSInteger minute,NSInteger second))completeBlock
{
    ...
    if (_timer==nil) {
        NSNumber *time = [NSNumber numberWithLongLong:timeStamp];
        //转换成NSTimeInterval,用longLongValue，防止溢出
        NSTimeInterval nsTimeInterval = [time longLongValue];
        __block int timeout = nsTimeInterval; //倒计时时间
        if (timeout!=0) {
