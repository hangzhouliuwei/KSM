...
{
    NSDictionary *pointDic = @{@"calcographyF":startTime,
                               @"biolysisF":@"1",
                               @"jactancyF":@"21",
                               @"hateworthyF":@([BagLocationManager shareInstance].latitude),//经度
                               @"apoenzymeF":[NSDate br_timestamp],
                               @"clapperclawF":[NSObject getIDFV],
                               @"reinhabitF":@([BagLocationManager shareInstance].longitude),//纬度
    };
    ...
    [service startWithCompletionBlockWithSuccess:^(__kindof BagBaseRequest * _Nonnull request) {
        if (request.response_code == 0) {
            ...
            [[BagUserManager shareInstance] updateUserModelWithDic:data[@"daemonF"][@"hematopoiesisF"]];
