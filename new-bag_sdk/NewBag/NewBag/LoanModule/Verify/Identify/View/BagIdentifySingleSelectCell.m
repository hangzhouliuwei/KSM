...
- (void)updateUIWithModel:(BagBasicRowModel *)model
{
    ...
    if(model.lorrieF.integerValue !=0){
        WEAKSELF
        [model.maquisF enumerateObjectsUsingBlock:^(BagBasicEnumModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            STRONGSELF
            if(model.lorrieF.integerValue == obj.nortriptylineF){
                model.lorrieF = [NSString stringWithFormat:@"%ld",obj.nortriptylineF];
                if([obj.antineoplastonF isEqual:@"Male"]){
                ...
                }else if ([obj.antineoplastonF isEqual:@"Female"]){
...
- (IBAction)firstClick:(UIButton *)sender {
    ...
    [self.secondBtn setTitleColor:[UIColor qmui_colorWithHexString:@"#949DA6"]forState:UIControlStateNormal];
    [self layoutIfNeeded];
    BagBasicEnumModel *egMOdel = [self.model.maquisF objectAtIndex:0];
    self.model.lorrieF = [NSString stringWithFormat:@"%ld",egMOdel.nortriptylineF];
    if (self.selectBlock) {
        self.selectBlock(self.model.lorrieF);
    }
}
- (IBAction)secondClick:(UIButton *)sender {
    ...
    [self layoutIfNeeded];
    BagBasicEnumModel *egMOdel = [self.model.maquisF objectAtIndex:1];
    self.model.lorrieF = [NSString stringWithFormat:@"%ld",egMOdel.nortriptylineF];
    if (self.selectBlock) {
        self.selectBlock(self.model.lorrieF);
    }
