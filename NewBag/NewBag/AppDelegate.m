...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    config.baseUrl = @"http://api-cti.ph.dev.ksmdev.top/api/";//配置请求域名地址
//    //拼接通用参数到链接
...
//    [config addUrlFilter:filter];
    ...
    self.window.rootViewController = [[BagRouterManager shareInstance] rootVC];
    //强制登录
    [self forceLogin];
    ...
    if (!BagUserManager.shareInstance.isLogin) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [[BagRouterManager shareInstance] jumpLoginWithSuccessBlock:^{
                ...
                [Bugly setUserIdentifier:BagUserManager.shareInstance.username];
            }];
