...
- (void)goSettingAction{
    BagMeSettingVC *set = [BagMeSettingVC new];
    ...
    if (indexPath.row == 0) {
        if (self.overdue) {
            ...
            cell.overdueClickBlock = ^{
                if (![weakSelf.model.careermanF.nonvoterF br_isBlankString]) {
                    BagWebViewController *web = [BagWebViewController new];
                    web.url = weakSelf.model.careermanF.nonvoterF;
                    [weakSelf.navigationController qmui_pushViewController:web animated:YES completion:nil];
                }
            };
            [cell updateUIWithProductName:weakSelf.model.careermanF.insolvableF logoUrl:weakSelf.model.careermanF.wholenessF amount:weakSelf.model.careermanF.percurrentF repayDate:weakSelf.model.careermanF.alchemicalF];
            return cell;
        }
    }
    BagMeListCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass(BagMeListCell.class)];
    BagMeListItemModel *model = self.model.antidiphtheriticF[indexPath.row - (self.model.careermanF ? 1 : 0)];
    [cell updateUIWithTitle:model.mudslingerF iconUrl:model.wholenessF];
...
{
    return self.overdue ? 1 + self.model.antidiphtheriticF.count : self.model.antidiphtheriticF.count;
}
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.overdue) {
        if (self.model.antidiphtheriticF.count > 1) {
            if (indexPath.row == 0) {
                if (![self.model.careermanF.nonvoterF br_isBlankString]) {
                    BagWebViewController *web = [BagWebViewController new];
                    web.url = self.model.careermanF.nonvoterF;
                    [self.navigationController qmui_pushViewController:web animated:YES completion:nil];
                }
                return;
            }
            BagMeListItemModel *model = self.model.antidiphtheriticF[indexPath.row-1];
            BagWebViewController *web = [BagWebViewController new];
            web.url = model.nonvoterF;
            [self.navigationController qmui_pushViewController:web animated:YES completion:nil];
        }
    }else{
        BagMeListItemModel *model = self.model.antidiphtheriticF[indexPath.row];
        BagWebViewController *web = [BagWebViewController new];
        web.url = model.nonvoterF;
        web.leftTitle = @"";
        [self.navigationController qmui_pushViewController:web animated:YES completion:nil];
