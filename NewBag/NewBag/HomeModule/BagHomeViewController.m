...
- (void)setupUI{
    ...
    UIImageView *topBcakImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"home_bg"]];
    topBcakImage.contentMode = UIViewContentModeScaleAspectFill;
    [self.view addSubview:topBcakImage];
...
{
    self.dataArray = data;
    self.jumpUrl = model.voyagerF;
    self.icon.hidden = self.titleLabel.hidden = YES;
   
    [self.tableView reloadData];
    [self.dragView.imageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",model.eskarF]] placeholderImage:[UIImage imageNamed:@"icon-kf"] completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
        if(!image)return;
...
{
    if ([NotNull(model.bishopF) br_isBlankString]) {
        return;
    }
    ...
    view.tapBlock = ^{
        ...
        [self jumpWeb:model.nonvoterF];
    };
    [view updateUIWithIconUrl:model.bishopF];
    [view show];
}
//跳 web
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([model.cellId isEqual:@"BANNER"]) {
        ...
        cell.bannerClickBlock = ^(BagHomeBannerItemModel * _Nonnull model) {
            STRONGSELF
            if([model.nonvoterF br_isBlankString])return;
            BagWebViewController *webVC = [[BagWebViewController alloc] init];
            webVC.url = model.nonvoterF;
    ...
    if ([model.cellId isEqual:@"LARGE_CARD"]) {
        ...
        cell.applyClickBlock = ^{
            [self applyClickWithProductId:((BagHomeBigCardItemModel *)model).franticF];
        };
    ...
    if ([model.cellId isEqual:@"RIDING_LANTERN"]) {
...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        [_tableView registerNib:[UINib nibWithNibName:NSStringFromClass(BagHomeBigCardTableViewCell.class) bundle:[NSBundle mainBundle]] forCellReuseIdentifier:NSStringFromClass(BagHomeBigCardTableViewCell.class)];
        [_tableView registerNib:[UINib nibWithNibName:NSStringFromClass(BagHomeSmallCardCell.class) bundle:[NSBundle mainBundle]] forCellReuseIdentifier:NSStringFromClass(BagHomeSmallCardCell.class)];
        [_tableView registerNib:[UINib nibWithNibName:NSStringFromClass(BagHomeProductListCell.class) bundle:[NSBundle mainBundle]] forCellReuseIdentifier:NSStringFromClass(BagHomeProductListCell.class)];
        [_tableView registerNib:[UINib nibWithNibName:NSStringFromClass(BagHomeBrandCell.class) bundle:[NSBundle mainBundle]] forCellReuseIdentifier:NSStringFromClass(BagHomeBrandCell.class)];
    }
    return _tableView;
}
