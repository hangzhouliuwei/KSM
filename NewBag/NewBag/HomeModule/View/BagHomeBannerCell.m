...
- (void)updateUIWithModel:(BagHomeBannerModel *)model
{
    ...
    [model.hematopoiesisF enumerateObjectsUsingBlock:^(BagHomeBannerItemModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [imgsArr addObject:obj.oldowanF];
    }];
    _model = model;
...
{
    if(self.bannerClickBlock && self.model.hematopoiesisF.count > index){
        BagHomeBannerItemModel *itemModel = [self.model.hematopoiesisF objectAtIndex:index];
...
- (SDCycleScrollView *)bannerView{
    if(!_bannerView){
        _bannerView = [SDCycleScrollView cycleScrollViewWithFrame:CGRectMake(0, 0, kScreenWidth - 28, 125) delegate:self placeholderImage:[UIImage imageNamed:@"home_banner_ep"]];
