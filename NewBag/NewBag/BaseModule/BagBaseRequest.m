...
- (void)requestWillStart:(YTKRequest *)request
{
    ...
    NSLog(@"\n=================\nXBBaseRequest:\n >>>>>>%@ host=%@\n url=%@ \n=============== param=%@",request.requestMethod == 0 ? @"GET" : @"Post",YTKNetworkConfig.sharedConfig.baseUrl,url,param);
}
...
{
    NSString *code = self.responseObject[@"imeafourteensurabilityNc"];
    return code.integerValue;
}
- (NSString *)response_message
{
    NSString *message = self.responseObject[@"frwnfourteenNc"];
    return message;
}
- (NSDictionary *)response_dic
{
    return self.responseObject[@"viusfourteenNc"];
}
- (void)hiddenLoading
{
