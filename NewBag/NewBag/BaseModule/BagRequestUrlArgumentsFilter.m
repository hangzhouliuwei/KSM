...
+ (NSDictionary *)getURLParam{
    NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithDictionary:@{@"pithlessF":@"ios",//
                                                                               @"quaysideF":[[[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleShortVersionString"] br_PramsString],//版本号
                                                                               @"cathecticizeF":[[NSObject getMobileStyle] br_PramsString].stringByRemovingPercentEncoding,//手机型号
                                                                               @"froglingF":NotNull([NSObject getIDFV]),//idfv
                                                                               @"sonderkommandoF":[[UIDevice currentDevice] systemVersion],//手机系统
                                                                               @"megabyteF":@"ph",
                                                                               @"signalF":[NSBundle mainBundle].bundleIdentifier,//包名
                                                                             }];
    [NSObject getIdfa:^(NSString * _Nonnull idfa) {
        [dic setObject:NotNull(idfa) forKey:@"indecentlyF"];
    }];
    [dic setObject: NotNull([BagUserManager shareInstance].sessionid) forKey:@"icicleF"];//session
    [dic setObject: NotNull([BagUserManager shareInstance].username) forKey:@"existentialF"];//手机号
    return  dic.copy;
}
