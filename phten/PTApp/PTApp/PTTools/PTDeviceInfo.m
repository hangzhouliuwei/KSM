...
@implementation PTDeviceInfo

+ (NSString *)version 
{
    NSDictionary *infoDic = [[NSBundle mainBundle] infoDictionary];
    NSString *appVersion = [infoDic objectForKey:@"CFBundleShortVersionString"];
    return PTNotNull(appVersion);
}
...
+ (NSString *)buildVersion {
    ...
    return PTNotNull(appBuildVersion);
}
//
+ (NSString *)systemVersion {
    NSString *systemVersion = [[UIDevice currentDevice] systemVersion];
    return PTNotNull(systemVersion);
}
+ (NSString *)idfa {
    NSString *uuid = [[ASIdentifierManager sharedManager].advertisingIdentifier UUIDString];
    return PTNotNull(uuid);
}
+ (NSString *)idfv {
...
+ (NSString *)packageName {
    ...
    return PTNotNull(appName);
}

+ (NSString *)bundleId {
    NSString *bundleId = [[NSBundle mainBundle] bundleIdentifier];
    return PTNotNull(bundleId);
}
...
+ (NSString *)standardFormateMAC:(NSString *)MAC {
    ...
    return PTNotNull([formateMAC uppercaseString]);
}
...
{
    //读取账号中保存的内容
    NSString *fistTimeStr = [PTCache getcacheYYObjectWithKey:KEY_FistTime];
    
    if ([PTTools isBlankString:fistTimeStr])
    {//如果是第一次 肯定获取不到 这个时候就存储一个
        NSString *fistTimeStr = [PTTools getTime];//获取当前时间
        [PTCache cacheYYObject:fistTimeStr withKey:KEY_FistTime];
        return fistTimeStr;
    }
    else{
        return fistTimeStr;
...
+(NSDictionary*)getDevices
{
    NSDictionary *zotenaismNcDic = @{
                                    @"chtenysalidesNc":PTNotNull([self getAvailableDiskSize]),
                                    @"ebtendicNc":PTNotNull([self getTotalDiskSize]),
                                    @"sctenlimazelNc":PTNotNull([self getTotalMemorySize]),
                                    @"hitenackNc":PTNotNull([self getAvailableMemorySize]),
                                    };
    NSDictionary *battery_statusDic = @{
                                         @"detenlasseNc":@([self batteryLevel]),
                                         @"battery_status":@([self getBatteryStatus] ? 1 : 0 ),
                                         @"aktenavitNc":@([self isCharging] ? 1 : 0)
                                       };
    NSDictionary *hardwareDic = @{
                                  @"xetenotimeNc":PTNotNull([self systemVersion]),
                                  @"prtentubercularNc":@"iPhone",
                                  @"batenuvrihiNc":PTNotNull([self deviceName]),
                                  @"petenmeanceNc":@([self deviceHeight]),
                                  @"sttentusNc":@([self deviceWidth]),
                                  @"sotenenoidNc":PTNotNull([self deviceSize]),
                                  @"tetenraNc":@([self getDeviceProTime])
                                 };
    NSDictionary *watentyNcDic = @{
                                   @"brtenezinessNc":@"0",
                                   @"detenisableNc":@([self isSimulator] ? 1 : 0),
                                   @"sitennmanNc":@([self isJailbroken] ? 1 : 0)
                                 };
    NSDictionary *retenncounterNcDic = @{
                                      @"ovtenrexertNc": PTNotNull([self timeZone]),
                                      @"pltentiniferousNc":@([self isUseProxy] ? 1 : 0),
                                      @"sutenmergibleNc":@([self isUseVPN] ? 1 : 0),
                                      @"cotennsellorNc":PTNotNull([self getSIMCardInfo]),
                                      @"matennicideNc":PTNotNull([self idfv]),
                                      @"tutenedoNc":PTNotNull([self deviceLanuage]),
                                      @"letenellingNc":PTNotNull([self networkStatus]),
                                      @"batenhlykNc":@(1),
                                      @"detenodulateNc":PTNotNull([self ip]),
                                      @"patenturageNc":PTNotNull([self idfa]),
                                     };
    
    NSDictionary *patenricentricNcDic = @{
                                          @"mitentimeNc":PTNotNull([self getBSSID]),
                                          @"frtenscatiNc":PTNotNull([self getWifiName]),
                                          @"kotenobehNc":PTNotNull([self getBSSID]),
                                          @"uptenornNc":PTNotNull([self getWifiName]),
                                         };
    NSDictionary *eatenternizeNcDic = @{@"patenricentricNc":@[patenricentricNcDic]};
    
    NSDictionary *dic = @{
                          @"zotenaismNc": zotenaismNcDic,
                          @"battery_status":battery_statusDic,
                          @"hardware":hardwareDic,
                          @"watentyNc":watentyNcDic,
                          @"retenncounterNc":retenncounterNcDic,
                          @"eatenternizeNc":eatenternizeNcDic
                         };
