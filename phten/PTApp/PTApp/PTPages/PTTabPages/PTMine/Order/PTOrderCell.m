...
- (void)configUIWithModel:(PTOrderListModel *)model
{
    _titleLabel.text = model.motenosyllabismNc;
    [_iconImage sd_setImageWithURL:[NSURL URLWithString:model.sitenhouetteNc]];
    self.amountLabel.text = model.istentacNc;

    self.statusLabel.text = model.latenarckianNc;

    self.statusLabel.textColor = [UIColor qmui_colorWithHexString:model.imtenotenceNc];
    if (![PTNotNull(model.matenanNc) br_isBlankString]) {
        self.repayBtn.hidden = NO;
        self.repayDateLabel.hidden = NO;
        self.repayDateDescLabel.hidden = NO;
        if ([PTNotNull(model.exteneriencelessNc) isEqual:@""]) {
            self.repayDateLabel.text =@"-";
        }else{
            self.repayDateLabel.text = model.exteneriencelessNc;
        }       
        self.repayBtn.backgroundColor = [UIColor br_colorWithHexString:model.shtenkariNc];
        [self.repayBtn setTitle:model.matenanNc forState:UIControlStateNormal];
    }else{
        self.repayBtn.hidden = YES;
        if ([PTNotNull(model.exteneriencelessNc) isEqual:@""]) {
            self.repayDateLabel.hidden = YES;
            self.repayDateDescLabel.hidden = YES;
        }else{
            self.repayDateLabel.hidden = NO;
            self.repayDateLabel.text = model.exteneriencelessNc;
        }
