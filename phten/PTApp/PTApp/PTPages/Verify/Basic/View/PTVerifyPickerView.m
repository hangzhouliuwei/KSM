...
@interface PTVerifyPickerView()<UITableViewDelegate,UITableViewDataSource>
...
@end
@implementation PTVerifyPickerView
...
- (void)setupUI{
    ...
    CGFloat height = [self.headerTitle br_getTextHeight:[UIFont qmui_mediumSystemFontOfSize:16] width:kScreenWidth - 80];
    

    UIView *header = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 64)];
    [self.bgView addSubview:header];
    [header br_setGradientColor:PTUIColorFromHex(0xDCF9AA) toColor:PTUIColorFromHex(0xffffff) direction:BRDirectionTypeTopToBottom bounds:CGRectMake(0, 0, kScreenWidth, 64)];
    
    UILabel *tipLabel = [[UILabel alloc] qmui_initWithFont:[UIFont qmui_mediumSystemFontOfSize:16] textColor:[UIColor blackColor]];
    tipLabel.textAlignment = NSTextAlignmentLeft;
    tipLabel.numberOfLines = 0;
    tipLabel.frame = CGRectMake(20, 16.f, kScreenWidth - 80, height);
...
{
    PTVerifyPickerCell *cell  =[tableView dequeueReusableCellWithIdentifier:NSStringFromClass(PTVerifyPickerCell.class)];
    id model = self.titles[indexPath.section];
    if ([model isKindOfClass: PTBaseModel.class]) {
        PTBasicEnumModel *enumModel = (PTBasicEnumModel *)model;
        [cell updateUIWithModel:enumModel.uptenornNc isSelected:indexPath.section == self.selectIndex];
    }
...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        [_tableView registerClass:[PTVerifyPickerCell class] forCellReuseIdentifier:NSStringFromClass(PTVerifyPickerCell.class)];
