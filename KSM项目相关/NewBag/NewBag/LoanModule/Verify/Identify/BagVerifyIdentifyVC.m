...
- (void)onNextClick{
    ...
    for (BagBasicRowModel *obj in self.datalModel.xathfourteenosisNc) {
        if(obj.tapafourteenxNc.integerValue == 0){
            if([obj.daryfourteenmanNc br_isBlankString]){
                
                [BagTrackHandleManager trackAppEventName:@"af_cc_cer_start_err" withElementParam:@{@"tag":NotNull(obj.imeafourteensurabilityNc)}];
                [self showToast:[NSString stringWithFormat:@"Please complete %@", NotNull(obj.orinfourteenarilyNc)] duration:1.5];
                return;
            }else{
                [dic setValue:obj.daryfourteenmanNc forKey:NotNull(obj.imeafourteensurabilityNc)];
                continue;
            }
        }else{
            [dic setValue:NotNull(obj.daryfourteenmanNc) forKey:NotNull(obj.imeafourteensurabilityNc)];
            continue;
        }
    }
    ...
    dic[@"seiafourteenbstractNc"] = NotNull(self.datalModel.paalfourteenympicsNc ? : self.datalModel.daryfourteenmanNc);
...
{
    _model = model;
    _datalModel = model.casafourteenbNc;
    self.countDownView.countTime = model.paeofourteengrapherNc;
    self.tableView.tableHeaderView = self.header;
    if ([model.casafourteenbNc.relofourteenomNc br_isBlankString]) {
        [self.header selectCardAction];
    }else{
        [self.header updateIDcardImageUrl:model.casafourteenbNc.relofourteenomNc];
    }
    self.nextBtn.hidden = self.datalModel.xathfourteenosisNc.count > 0 ? NO : YES;
...
{
    BagBasicRowModel *model = [self.datalModel.xathfourteenosisNc objectAtIndex:indexPath.section];
    ...
    if([model.cellType isEqualToString:@"txt"]){
        ...
        cell.textBlock = ^(NSString * _Nonnull str) {
            STRONGSELF
            model.daryfourteenmanNc = NotNull(str);
            [strongSelf tarckSelectResultTagString:model.imeafourteensurabilityNc contentString:str];
        };
        cell.textBeginBlock = ^{
            STRONGSELF
            [strongSelf tarckClickTagString:model.imeafourteensurabilityNc];
        };
        return cell;
    }
    if([model.cellType isEqualToString:@"day"]){
        ...
        cell.clickBlock = ^{
            ...
            datePickerView.title = model.fldgfourteeneNc;
            ...
            datePickerView.resultBlock = ^(NSDate *selectDate, NSString *selectValue) {
                ...
                model.daryfourteenmanNc = date;
                [self.tableView reloadData];
                [self tarckSelectResultTagString:model.imeafourteensurabilityNc contentString:model.daryfourteenmanNc];
...
{
    return self.datalModel.xathfourteenosisNc.count;
}
...
- (BagIdentifyHeaderView *)header
{
    if (!_header) {
        ...
        _header.selectTypeBlock = ^{
            ...
            BagVerifyPickerView *picker = [[BagVerifyPickerView alloc] initWithTitleArray:weakSelf.model.casafourteenbNc.tubofourteendrillNc headerTitle:@"Select An ID Type"];
            picker.clickBlock = ^(BagIdentifyListModel *model) {
                [weakSelf.header updateModel:model];
                [BagTrackHandleManager trackAppEventName:@"af_cc_result_cer_type" withElementParam:@{@"type:":@(model.ceNcfourteen)}];
            };
