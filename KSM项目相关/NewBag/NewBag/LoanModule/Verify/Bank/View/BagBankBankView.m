...
- (void)updateUIWithModel:(BagBankBankModel *)model
{
    ...
    if (![model.koNcfourteen.ovrcfourteenutNc br_isBlankString] && model.koNcfourteen.blthfourteenelyNc != 0) {
        self.bankAccountTextfield.text = model.koNcfourteen.ovrcfourteenutNc;
        WEAKSELF
        [model.unrdfourteenerlyNc enumerateObjectsUsingBlock:^(BagBankItemModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if(obj.regnfourteenNc == model.koNcfourteen.blthfourteenelyNc){
                ...
                strongSelf.bankTitleLabel.text = NotNull(strongSelf.selectModel.uporfourteennNc);
...
- (void)bankListClick{
    [self endEditing:YES];
    BagVerifyPickerView *picker = [[BagVerifyPickerView alloc] initWithTitleArray:self.model.unrdfourteenerlyNc headerTitle:@"Bank card selection"];
    picker.clickBlock = ^(BagBankItemModel *model) {
        self.selectModel = model;
        self.bankTitleLabel.text = NotNull(self.selectModel.uporfourteennNc);
        ...
        [BagTrackHandleManager trackAppEventName:@"af_pub_result_bank" withElementParam:@{@"type":@(self.selectModel.regnfourteenNc)}];
