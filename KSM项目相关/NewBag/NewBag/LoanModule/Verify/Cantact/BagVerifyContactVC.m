...
- (void)onNextClick{
    ...
    [self.model.ovrffourteenraughtNc enumerateObjectsUsingBlock:^(BagContactItmeModel * _Nonnull model, NSUInteger idx, BOOL * _Nonnull stop) {
        [dic setValue:model.koNcfourteen.uporfourteennNc forKey:NotNull(model.inhofourteenationNc[0][@"uporfourteennNc"])];
        [dic setValue:model.koNcfourteen.halofourteenwNc forKey:NotNull(model.inhofourteenationNc[1][@"uporfourteennNc"])];
        [dic setValue:@(model.koNcfourteen.bedifourteeneNc) forKey:NotNull(model.inhofourteenationNc[2][@"uporfourteennNc"])];
    }];
    
    NSDictionary *pointDic = @{
                                @"deamfourteenatoryNc": @(self.startTime.doubleValue),
                                @"munifourteenumNc": NotNull(self.productId),
                                @"hyrafourteenrthrosisNc":@"23",
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BagContactItmeModel *model = self.model.ovrffourteenraughtNc[indexPath.section];
    ...
    cell.relationClick = ^{
        ...
        BagVerifyPickerView *picker = [[BagVerifyPickerView alloc] initWithTitleArray:model.bedifourteeneNc headerTitle:model.fldgfourteeneNc];
        picker.clickBlock = ^(BagContactRelationEnumModel  *enumModel) {
            
            [BagTrackHandleManager trackAppEventName:@"af_cc_result_contact_item" withElementParam:@{@"index":@(indexPath.section),@"content":NotNull(enumModel.uporfourteennNc)}];

            model.koNcfourteen.bedifourteeneNc = enumModel.dempfourteenhasizeNc;
...
{
    return self.model.ovrffourteenraughtNc.count;
}
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    BagContactItmeModel *model = self.model.ovrffourteenraughtNc[section];
    ...
    label.text = model.fldgfourteeneNc;
...
- (void)contactPicker:(CNContactPickerViewController *)picker didSelectContact:(CNContact *)contact {
    ...
    for (CNLabeledValue *phone in contact.phoneNumbers) {
        ...
        self.selectModel.koNcfourteen.uporfourteennNc = NotNull(name);
        self.selectModel.koNcfourteen.halofourteenwNc = NotNull(phoneString);        
    }
    [self.tableView reloadData];
}
#pragma mark - getter
