...
- (void)updateUIWithModel:(BagBasicRowModel *)model
{
    ...
    if(![model.daryfourteenmanNc br_isBlankString]){
        ...
        self.isSet = YES;
        self.textField.text = NotNull(model.daryfourteenmanNc);
    }else{
        CGFloat width = [model.fldgfourteeneNc br_getTextWidth:[UIFont systemFontOfSize:14] height:48];
        if (!self.isSet) {
            self.textFieldLeftConst.constant = 14+width + 5;
        }
        self.textField.text = @"";
    }
    NSMutableAttributedString *attrString = [[NSMutableAttributedString alloc] initWithString:NotNull(model.imeafourteensurabilityNc) attributes: @{NSForegroundColorAttributeName:[UIColor grayColor], NSFontAttributeName:[UIFont systemFontOfSize:12]} ];
    self.textField.attributedPlaceholder = attrString;
    self.textField.keyboardType = model.techfourteenedNc.integerValue == 1 ? UIKeyboardTypeNumberPad : UIKeyboardTypeDefault;
    if (![model.imeafourteensurabilityNc isEqualToString:@"email"]) {
        self.tableView = nil;
    }
}
