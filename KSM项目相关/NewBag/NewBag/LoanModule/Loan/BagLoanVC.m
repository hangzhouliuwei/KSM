...
#pragma mark - next
- (void)onNextClick{
    if ([NotNull(self.model.heisfourteentopNc.relofourteenomNc) br_isBlankString]) {
        [self.presenter sendOrderPushRequestWithOrderId:BagUserManager.shareInstance.order_numer product_id:self.productId];
        return;
    }
    [[BagRouterManager shareInstance] routeWithUrl:[self.model.heisfourteentopNc.relofourteenomNc br_pinProductId:self.productId]];
}
#pragma mark - BagHomePresenterProtocol
- (void)updateUIWithModel:(BagLoanDeatilModel *)model
{
    _model = model;
    _dataArray = model.atesfourteeniaNc;
...
{
    BagVerifyItemModel *model = [self.model.atesfourteeniaNc objectAtIndex:indexPath.row];
    if(model.frllfourteenyNc){
        [[BagRouterManager shareInstance] routeWithUrl:[model.relofourteenomNc br_pinProductId:_productId]];
    }else{
        [[BagRouterManager shareInstance] routeWithUrl:[self.model.heisfourteentopNc.relofourteenomNc br_pinProductId:_productId]];
    }
}
#pragma mark - getter
