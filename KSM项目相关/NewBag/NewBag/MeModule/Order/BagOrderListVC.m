...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.dataArray.count > indexPath.row) {
        ...
        cell.refundBtnClickBlock = ^{
            [[BagRouterManager shareInstance] routeWithUrl:model.aplifourteencableNc];
        };
...
{
    if([model.aplifourteencableNc containsString:@"http://"] || [model.aplifourteencableNc containsString:@"https://"]){
        BagWebViewController *webVC = [[BagWebViewController alloc] init];
        webVC.url = NotNull(model.aplifourteencableNc);
        [self.navigationController qmui_pushViewController:webVC animated:YES completion:nil];
        return;
    }//认证完成直接跳转web
    if(model.detrfourteenogyrateNc && model.munifourteenumNc.integerValue !=0){
        BagLoanVC *loanRecordVC = [[BagLoanVC alloc] init];
        loanRecordVC.productId = NotNull(model.munifourteenumNc);
        [self.navigationController qmui_pushViewController:loanRecordVC animated:YES completion:nil];
        return;
    }
    [self.homePresenter sendGetProductDetailRequestWithProductId:model.aplifourteencableNc];
}
